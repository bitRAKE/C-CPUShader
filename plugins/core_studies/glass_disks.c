...
{
    float distance = shader_ray_circle(ro, rd, LIGHT_CENTER, LIGHT_RADIUS);
...
static void set_glass_hit(
    ...
    float absorption)
{
    float distance = shader_ray_circle(ro, rd, center, radius);
...
vec4_t glass_disks_main(vec2_t fragCoord, const shader_uniforms_t *uniforms) {
    ...
    float angle = 2.0f * PI * ((float)frame + shader_rand_1(&state)) / DIRECTIONAL_SAMPLES;
    vec2_t dir = vec2(cosf(angle), sinf(angle));

    float wavelength = shader_rand_1(&state);

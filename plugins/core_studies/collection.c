...
static const shader_desc_t g_shaders[] = {
    THREE_FIRE_SHADER(ENTRY)
    SPHERE_TRACING_SHADER(ENTRY)
    KINETIC_ORBS_SHADER(ENTRY)
    GLASS_LENSES_SHADER(ENTRY)
    GLASS_DISKS_SHADER(ENTRY)
    CRYSTAL_HALL_SHADER(ENTRY)
    DICE_SHADER(ENTRY)
};
...
{
    g_info.collection_name    = "Core Studies";
    g_info.collection_version = "1.0";
    g_info.collection_blurb   = "Visual studies covering fire, ray marching, optics, and procedural geometry.";

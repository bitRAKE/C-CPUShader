/*
 * u_texture.h -- SDK texture sampling utilities for plugins.
 *
 * SDK version of src/u_texture.h. Provides buffer lookup and texture
 * sampling functions. All functions are static inline so plugins need
 * no link dependency on the host.
 */
...
/* Inline label comparison -- avoids strcmp / <string.h> dependency. */
static inline bool u_texture_label_eq(const char *a, const char *b)
{
    if (a == NULL || b == NULL) {
        return false;
    }
...
/* Inline buffer lookup -- SDK replacement for shader_buffers_find. */
static inline const shader_buffer_t *shader_buffers_find(const shader_buffers_t *buffers, const char *label)
{
    ...
    for (int index = 0; index < buffers->count; index++) {
        if (u_texture_label_eq(buffers->items[index].label, label)) {

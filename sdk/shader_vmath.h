/*
 * shader_vmath.h -- Vector math library for shader plugins.
 *
 * SDK copy of src/vmath.h. Content is identical.
 * Define VMATH_IMPL before including this header in *one* C file
...
vec3_t v3_add1(vec3_t a, float b);
...
vec3_t v3_saturate(vec3_t v);
vec3_t v3_safe_normalize(vec3_t v);
...
vec2_t v2_rotate(vec2_t p, float angle);
float  safe_tanhf(float x);

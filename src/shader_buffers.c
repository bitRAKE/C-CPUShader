    ...
    size_t error_size)
{
    ...
    if (FAILED(hr)) {
        shader_buffers_set_error(error, error_size, "Failed to read frame 0 from '%s' (hr=0x%08lX).", path, (unsigned long)hr);
        goto fail;
    }

    hr = IWICBitmapFrameDecode_GetSize(frame, &width, &height);
    if (FAILED(hr) || width == 0 || height == 0) {
        shader_buffers_set_error(error, error_size, "Failed to query texture size for '%s'.", path);
        goto fail;
    }

    if ((size_t)width > (SIZE_MAX / (size_t)bytes_per_pixel) || (size_t)height > (SIZE_MAX / ((size_t)width * (size_t)bytes_per_pixel))) {
        shader_buffers_set_error(error, error_size, "Texture '%s' is too large.", path);
        goto fail;
    }
    ...
    if (data == NULL) {
        shader_buffers_set_error(error, error_size, "Failed to allocate %zu bytes for texture '%s'.", size_bytes, path);
        goto fail;
    }
    ...
    if (FAILED(hr)) {
        shader_buffers_set_error(error, error_size, "Failed to convert texture '%s' to requested format (hr=0x%08lX).", path, (unsigned long)hr);
        goto fail;
    }

    hr = IWICFormatConverter_CopyPixels(converter, NULL, (UINT)row_stride_bytes, (UINT)size_bytes, (BYTE *)data);
    if (FAILED(hr)) {
        shader_buffers_set_error(error, error_size, "Failed to copy pixels from '%s' (hr=0x%08lX).", path, (unsigned long)hr);
        goto fail;
    }

...
static ShaderEntry g_shaders[] = {
    {"sphere_tracing", sphere_tracing_main},
    {"glass_lenses", glass_lenses_main},
    {"glass_disks", glass_disks_main},
    {"crystal_hall", crystal_hall_main},
    {"master_class", master_class_main}
};

static const int g_shader_count = (int)(sizeof(g_shaders) / sizeof(g_shaders[0]));
static int       g_current_shader_index = 4;
...
{
    return g_current_shader(fragCoord, resolution, time, frame);
}
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nShowCmd) {
    ...
    window_set_shader_switcher(shader_cycle, shader_current_name);

    //Render Loop with 31 total render workers, including the main thread
    window_run(main_image, 31, true);

...
static vec3_t hsv2rgb(vec3_t c)
{
    vec3_t p = v3_abs(v3_sub(v3_mul1(v3_fract(v3_add(vec3_o(c.x), vec3(1.0f, 2.0f / 3.0f, 1.0f / 3.0f))), 6.0f), vec3_o(3.0f)));
    vec3_t q = vec3(
        clampf(p.x - 1.0f, 0.0f, 1.0f),
        clampf(p.y - 1.0f, 0.0f, 1.0f),
        clampf(p.z - 1.0f, 0.0f, 1.0f));

    return v3_mul1(v3_lerp(vec3_o(1.0f), q, c.y), c.z);
}
...
vec3_t hsv_picker_rgb(const hsv_picker_state_t *state)
{
    ...
    return hsv2rgb(vec3(value.hue, value.sat, value.val));
}
    ...
    hsv_picker_region_t active_region)
{
    ...
    vec3_t ring_col = hsv2rgb(vec3(hue_on_ring, 1.0f, 1.0f));
    ...
    hue_col = hsv2rgb(vec3(value.hue, 1.0f, 1.0f));
    ...
    sel_col = hsv2rgb(vec3(value.hue, value.sat, value.val));
    comp_col = hsv2rgb(vec3((value.hue + 0.5f) - floorf(value.hue + 0.5f), 1.0f, 1.0f));

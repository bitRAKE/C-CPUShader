...
static vec3_t hsv2rgb_sdr(vec3_t c)
{
...
vec4_t colorspace_sdr_ui_main(vec2_t fragCoord, const shader_uniforms_t *uniforms)
{
    ...
    hue_color = hsv2rgb_sdr(vec3(uv.x, 0.88f, 0.98f));
    ...
    if (rect_mask(uv, vec2(0.545f, 0.28f), vec2(0.900f, 0.72f), 0.006f) > 0.0f) {
        ...
        vec3_t wheel = hsv2rgb_sdr(vec3(hue, saturate(radius * 1.05f), 0.98f));
        vec3_t fill_color = hsv2rgb_sdr(vec3(hue, 0.48f, 0.94f));

...
Build the standalone tool from the repo root with:

```powershell
cmd /c pocs\hsv_picker\_build.cmd
```

That produces:

- `pocs/hsv_picker/hsv_picker_tool.exe`

## What It Demonstrates

The standalone tool in `pocs/hsv_picker/hsv_picker_tool.c` reuses the picker shader logic from `pocs/hsv_picker/hsv_picker.c` directly. It does not re-implement the math in a second place. Instead, it provides a tiny host application that:
